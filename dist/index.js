...
const syncSubscription = (subscriptionData) => {
    return new Promise((resolve) => {
        vinted_api_1.default.search(subscriptionData.url, false, false, {
            per_page: '20'
        }).then((res) => {
            if (!res.items) {
            ...
            if (!lastItemTimestamp || newLastItemDate.getTime() > lastItemTimestamp) {
                (0, typeorm_1.getConnection)().manager.getRepository(database_1.Subscription).update({
            ...
            for (let item of itemsToSend) {
                ...
                client.channels.cache.get(subscriptionData.channelId).send({ embeds: [embed], components: [
...
const sync = async () => {
    ...
    const subscriptions = await (0, typeorm_1.getConnection)().manager.getRepository(database_1.Subscription).find({
        isActive: true
    });
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (!adminIDs.includes(interaction.user.id))
        return void interaction.reply(`:x: Vous ne disposez pas des droits pour effectuer cette action !`);
    switch (interaction.commandName) {
        case 'abonner': {
            ...
            (0, typeorm_1.getConnection)().manager.getRepository(database_1.Subscription).save(sub);
            interaction.reply(`:white_check_mark: Votre abonnement a été créé avec succès !\n**URL**: <${sub.url}>\n**Salon**: <#${sub.channelId}>`);
            break;
        }
        case 'désabonner': {
            const subID = interaction.options.getString('id');
            const subscription = await (0, typeorm_1.getConnection)().manager.getRepository(database_1.Subscription).findOne({
                isActive: true,
                id: parseInt(subID)
            });
            if (!subscription) {
